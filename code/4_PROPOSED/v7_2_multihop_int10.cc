...
// ============================================================================
// HYBRID PROTOCOL v7 -- CENTER, MULTIHOP (replaces the CH-to-CH chain with
// greedy multihop routing) + BACKUP CH FAILOVER
// ns-3.41 / C++
//
...
// RESULT of this exact file (seed 12345): FND 1421 / HND 1991 / LND 2193 / PDR 98.27%
...
//
// VARIANT requested to compare the linear PEGASIS-style CH-to-CH CHAIN used
// in v7_chain_backup_center.cc against a MULTIHOP routing tree instead:
//
//   Chain (v7_chain_backup_center.cc): CHs form ONE linear path (nearest-neighbor greedy
//   order), and only the single CH closest to the BS (the "gateway")
//   transmits to it; every other CH's data flows along that one path.
//
//   Multihop (this file): each CH independently picks its OWN next hop --
//   the nearest OTHER CH that is strictly closer to the BS than itself --
//   and forwards there; a CH with no such closer CH available transmits
//   directly to the BS. This forms a routing TREE (possibly several
//   branches converging toward the BS) rather than one single path, so a
//   CH's traffic can take a shorter, more direct route toward the BS
//   instead of being forced through the chain's specific visiting order.
//   No cycles are possible: forwarding always requires "closer to BS than
//   me," a strict partial order.
//
// Same 100x100 field, RANGE=25, two-slope radio, BS at field center (50,50)
// as v7_chain_backup_center.cc. SETUP_INTERVAL changed to 10 (was 5, matching v7_center)
// per request, to also see the interval effect on top of the chain-vs-
// multihop routing choice -- comparing against a still-unreleased interval=5
// multihop run would isolate the interval effect alone, and against
// v7_chain_backup_center.cc (chain, interval=5) isolates the routing-topology effect alone.
//
...
// every CH that was forwarding to the dead one is redirected to the backup
// (routing repair -- the multihop analogue of the chain's slot repair). If
// no valid backup exists, those CHs simply fall back to transmitting
// directly to the BS for the rest of the interval (no relay parent
// available), rather than losing their data outright.
//
...
static constexpr uint32_t SETUP_INTERVAL = 10; // CHANGED: re-cluster every 10 rounds (was 5)
...
// --------------------- NEW for this variant: greedy multihop routing -------
// Instead of one linear chain with a single gateway, every current CH
// independently picks its own next hop: the nearest OTHER alive CH that is
// strictly closer to the BS than itself. A CH with no such candidate (it is
// already the closest CH to the BS in its local neighborhood) gets the
// sentinel numeric_limits<uint32_t>::max(), meaning "transmit directly to
// the BS." Because every hop strictly reduces distance-to-BS, the resulting
// structure is guaranteed to be a forest of trees rooted at BS-facing CHs --
// no cycles are possible.
static void BuildMultihopRouting(const vector<SensorNode>& nodes, vector<uint32_t>& nextHop)
{
    nextHop.assign(nodes.size(), numeric_limits<uint32_t>::max());
    vector<uint32_t> chs;
    for (const auto& n : nodes) if (n.alive && n.finalCH) chs.push_back(n.id);

    for (uint32_t c : chs) {
        const double selfDBS = DistBS(nodes[c]);
        uint32_t best = numeric_limits<uint32_t>::max();
        double bestHop = numeric_limits<double>::infinity();
        for (uint32_t o : chs) {
            if (o == c) continue;
            if (DistBS(nodes[o]) >= selfDBS - 1e-9) continue; // must be strictly closer to BS
            const double d = Dist(nodes[c], nodes[o]);
            if (d < bestHop) { bestHop = d; best = o; }
        }
        nextHop[c] = best;
    }
}
                                 ...
                                 uint32_t setupIntervalLen)
{
    ...
    // NEW (backup + multihop): before disconnecting members (or losing a
    // relay parent), try promoting the pre-designated backup CH -- only if
    // it has enough energy to plausibly survive CH duty for the rest of the
    // interval. If the dead CH was relaying for other CHs in the multihop
    // routing tree, the backup inherits its EXACT role: every CH that was
    // forwarding to the dead one is redirected to the backup, and the
    // backup takes over the dead CH's own onward route (repairing the
    // routing tree instead of breaking it). If no valid backup exists,
    // those CHs fall back to transmitting directly to the BS, and the dead
    // CH's own members are disconnected.
    ...
    for (uint32_t c : orphanedCHs) {
        ...
        if (backupId != numeric_limits<uint32_t>::max()
            ...
            if (nodes[backupId].energy >= estCostPerRound) {
                ...
                // Repair the routing tree: the backup inherits the dead CH's
                // own onward route, and every other CH that was forwarding
                // to the dead CH is redirected to the backup instead.
                nextHop[backupId] = nextHop[c];
                for (uint32_t x = 0; x < N; ++x)
                    if (nodes[x].alive && nodes[x].finalCH && x != backupId && nextHop[x] == c) { nextHop[x] = backupId; ++r.chainRepairs; }
            } else {
        ...
        if (!promoted) {
            ...
            if (emergencyId != numeric_limits<uint32_t>::max()) {
                ...
                if (nodes[emergencyId].energy >= estCostPerRound) {
                    ...
                    nextHop[emergencyId] = nextHop[c];
                    for (uint32_t x = 0; x < N; ++x)
                        if (nodes[x].alive && nodes[x].finalCH && x != emergencyId && nextHop[x] == c) { nextHop[x] = emergencyId; ++r.chainRepairs; }
                }
            }
        }
        ...
        if (!promoted) {
            ...
            // No backup/emergency CH available: any CH that was forwarding
            // to the dead one falls back to transmitting DIRECTLY to the BS
            // for the rest of this interval, instead of losing its data --
            // the multihop analogue of the chain's "bypass" fallback.
            for (uint32_t x = 0; x < N; ++x)
                if (nodes[x].alive && nodes[x].finalCH && nextHop[x] == c) nextHop[x] = numeric_limits<uint32_t>::max();
        }
    }
    for (auto& n : nodes) {
        if (n.alive && n.clusterHead < N && !nodes[n.clusterHead].alive)
    ...
    // ---- NEW: multihop relay along the routing TREE built by
    // BuildMultihopRouting, instead of the single linear chain. Process the
    // currently alive CHs FARTHEST-from-BS first: since every nextHop
    // strictly reduces distance-to-BS, this guarantees every CH's incoming
    // (already-aggregated) traffic has arrived before that CH itself is
    // processed and forwards onward (or transmits to the BS, if it has no
    // next hop this round). ----
    vector<uint32_t> aliveCHs;
    for (uint32_t c = 0; c < N; ++c) if (nodes[c].alive && nodes[c].finalCH) aliveCHs.push_back(c);
    sort(aliveCHs.begin(), aliveCHs.end(),
         [&](uint32_t a, uint32_t b) { return DistBS(nodes[a]) > DistBS(nodes[b]); });

    for (uint32_t c : aliveCHs) {
        if (!nodes[c].alive || !nodes[c].finalCH) continue;
        const uint32_t nh = (c < nextHop.size()) ? nextHop[c] : numeric_limits<uint32_t>::max();
        const bool hasParent = (nh < N && nodes[nh].alive && nodes[nh].finalCH);

        if (hasParent) {
            const double d = Dist(nodes[c], nodes[nh]);
            const double tx = TxEnergy(PACKET_BITS, d);
            if (tx > nodes[c].energy) { nodes[c].energy = 0.0; nodes[c].alive = false; nodes[c].finalCH = false; continue; }
            nodes[c].energy -= tx;
            ++r.dataTx;
            const double rx = RxEnergy(PACKET_BITS);
            if (rx > nodes[nh].energy) { nodes[nh].energy = 0.0; nodes[nh].alive = false; nodes[nh].finalCH = false; continue; }
            nodes[nh].energy -= rx;
            ++r.dataRx;
            const uint32_t incoming = chainMsgCount[c];
            const double aggRelay = static_cast<double>(incoming) * AggEnergy(PACKET_BITS);
            if (aggRelay > nodes[nh].energy) { nodes[nh].energy = 0.0; nodes[nh].alive = false; nodes[nh].finalCH = false; continue; }
            nodes[nh].energy -= aggRelay;
            chainMsgCount[nh] += incoming;
            delaySum += DelayMs(PACKET_BITS, d);
        } else {
            // Root of its branch (no closer CH available this round, or its
            // assigned parent died with no replacement): transmit directly
            // to the BS -- by now chainMsgCount[c] already includes every
            // descendant's relayed traffic, since children were processed first.
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v7 CENTER, MULTIHOP, interval=10: Multihop Routing Tree + Backup CH Failover\n";
    cout << "========================================\n";
    ...
    cout << "NEW: each CH forwards to the nearest OTHER CH strictly closer to the BS (multihop\n";
    cout << "     routing tree/forest, possibly several roots), instead of one linear chain\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("v7_2_multihop_int10-clustering.xml");
    ...
    ofstream rounds("v7_2_multihop_int10-results.csv");
    ofstream energy("v7_2_multihop_int10-node-energy.csv");
    ofstream lifetime("v7_2_multihop_int10-node-lifetime.csv");
    ...
    rounds << "Round,Alive,Dead,CH_Count,Generated,Delivered,Lost,Unclustered,"
              ...
              "PDR,Round_Duration_s,Throughput_kbps,Backup_Promotions,Rejected_Promotions,Routing_Repairs,Partial_Promotions,Emergency_Promotions\n";
    ...
    for (uint32_t round = 1; round <= MAX_ROUNDS; ++round) {
        ...
        if (isSetupRound) {
            ...
            ++setupIndex;
            BuildMultihopRouting(nodes, nextHop);  // NEW for this variant
        }

        RoundResult r = SimulateRound(nodes, round, isSetupRound, nextHop, SETUP_INTERVAL);
    ...
    cout << "Routing Repairs via Backup = " << totalChainRepairs
         << " (CHs redirected to a promoted backup instead of losing their relay parent)\n";
    cout << "\nCSV outputs: v7_2_multihop_int10-results.csv, v7_2_multihop_int10-node-energy.csv,\n";
    cout << "             v7_2_multihop_int10-node-lifetime.csv, v7_2_multihop_int10-clustering.xml\n";
    cout << "========================================\n";
