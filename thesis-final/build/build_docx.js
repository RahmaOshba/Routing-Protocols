...
children.push(callout('Key results',[
 ...
 [t('BS at the field center: ',{b:true,size:20}),t('FND 2621 / HND 2711 / LND 2756 rounds, PDR 99.27% — the latest first-node death of every protocol (best reproduced protocol: SH-LEACH Improved, FND 1480), and the highest PDR of every protocol whose first node survives past round 2000. Several reproduced protocols (LEACH, PEGASIS, HEED, H-LEACH, EECH-HEED) deliver up to 0.5 pp more (99.30–99.74%), but their first node dies 1,100–2,000 rounds earlier.',{size:20})],
...
[ 'A reproduction of five published protocols (LEACH, HEED, PEGASIS, SH-LEACH, H-LEACH) plus EECH-HEED, first under each paper\'s own settings and then in one unified ns-3 environment, so that they can be compared fairly.',
  'Targeted corrections of two literature hybrids whose published algorithms could not work as described (H-LEACH deadlock, SH-LEACH ever-growing CH probability), plus a rotation fix for EECH-HEED.',
...
children.push(table(['Protocol (year)','Core mechanism','Strength','Limitation addressed in this work'],[
 ...
 ['SH-LEACH (2015) [4]','LEACH threshold × energy ratio with a CH-selection counter (Eq. 3)','Energy-balanced CH choice','Round factor never resets → ~29 CHs per round; no failure recovery'],
 ...
 ['EECH-HEED (2025) [6]','HEED + EECH zones + adaptive threshold sensing','Zone-aware election; sends only on significant change','Not a LEACH+HEED hybrid; used as context only'],
...
children.push(P('',{after:60}));
children.push(P([t('PHY/MAC sanity check. ',{b:true}),t('Before relying on the analytical energy model, a small IEEE 802.15.4 (LR-WPAN) scenario with real PHY/MAC layers, log-distance path loss and a per-state energy source was run in ns-3 (3 sensors + 1 sink, 1000 s): 300/300 packets delivered (PDR 100%), 120 bit/s throughput, 62.64 J consumed per node out of 100 J. All four nodes — sink included — spent the same energy because the receiver stays on (idle listening: 3.3 V × 19 mA × 999 s ≈ 62.6 J; transmitting took only ≈ 0.2 s in total). This is why clustering protocols let members sleep outside their TDMA slot, and why the 100-node study charges energy per transmitted and received bit.')]));
...
children.push(P('',{after:60}));
children.push(P([t('What the reproduction showed. ',{b:true}),t('PEGASIS matched its paper within 0.3% on LND and within 3.1% on FND and HND; LEACH matched the paper\'s LND within 1.1%. HEED was run with 500 nodes as in the paper, which reports graphs only. While checking the codes against the papers, a reporting bug was found and fixed: a run that hit the round limit used to report its last death as LND; all protocols now run until the last node really dies. H-LEACH, as literally written, never elects a CH: with equal initial energy no node is strictly above the average, so a fallback was required just to run it; changing the gate to ‘≥ average’ and adding LEACH\'s G-set (each node is CH at most once per 10-round epoch) improved FND/HND/LND by 6.3–6.4%. SH-LEACH\'s Eq. 3 multiplies the probability by the round number, which is never reset, so about 29 nodes become CH every round; making that factor periodic (r mod 1/Cprob), as in LEACH, raised FND from 647 to 1480 (+129%) and LND from 721 to 1558. HEED kept a high PDR (99.68%) but the earliest first-node death (FND 634), because its iterative CH negotiation is paid every round. EECH-HEED was rebuilt from its equations (node degree in Zone 2, the Eq. 6 dynamic threshold with its G set, relays that pay for forwarding, Eq. 7 sensing): in its own scenario its FND is within 10% of the paper. In the unified environment its sensing is switched off, because it only makes nodes send less; there, Eq. 5 gives Zone-2 nodes a probability near 1, so the 1/P rotation stops working. Rotating Zone 2 at the paper\'s own 10% CH target raised FND by 4.3% and LND by 14.4%. It is kept as context.')]));
