...
children.push(callout('Key results',[
 ...
 [t('BS at the field center: ',{b:true,size:20}),t('FND 2621 / HND 2711 / LND 2756 rounds, PDR 99.27% — the latest first-node death of every protocol reproduced, and the highest PDR of every protocol whose first node survives past round 2000 (H-LEACH Improved: FND 2347, PDR 97.33%; EECH-HEED with sensing: FND 2502, PDR 92.14%). LEACH, PEGASIS, HEED and SH-LEACH Improved deliver 0.1–0.5 pp more (99.40–99.79%), but their first node dies 1,200–2,000 rounds earlier.',{size:20})],
...
children.push(P('',{after:60}));
children.push(P([t('What the reproduction showed. ',{b:true}),t('PEGASIS matched its paper within 0.3% on LND and within 3.1% on FND and HND; LEACH matched the paper\'s LND within 1.1%. HEED was run with 500 nodes as in the paper, which reports graphs only. While checking the codes against the papers, a reporting bug was found and fixed: a run that hit the round limit used to report its last death as LND; all protocols now run until the last node really dies. H-LEACH, as literally written, never elects a CH: with equal initial energy no node is strictly above the average, so a fallback was required just to run it; relaxing the gate to 0.8× the average let the energy-weighted rule actually decide, improving FND/HND/LND by 7.8–8.6% at no PDR cost. SH-LEACH applied a single probability doubling instead of HEED\'s iterative doubling; correcting it improved HND, LND and PDR but lowered FND (fewer, larger clusters). HEED kept a high PDR (99.68%) but the earliest first-node death (FND 634), because its iterative CH negotiation is paid every round. EECH-HEED reaches the longest raw lifetime only because its adaptive sensing transmits less often, so it is kept as context.')]));
