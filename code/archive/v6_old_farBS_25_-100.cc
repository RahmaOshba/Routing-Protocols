...
// ============================================================================
// HYBRID PROTOCOL v6: CH-TO-CH CHAIN RELAY (Far-BS Scenario)
// ns-3.41 / C++
//
// SAME CH selection and multi-round reuse as v3 (single-pass energy x
// connectivity score, 5-round setup interval) -- see v3's own header for
// that design's rationale, unchanged here. v6 adds ONE new mechanism:
//
//   4. CH-TO-CH CHAIN RELAY (NEW): in v3, every CH transmits its
//      aggregated cluster data directly to the BS -- one long, expensive
//      hop per CH per round. v6 instead builds a PEGASIS-style greedy
//      chain among the (small) set of current CHs only, and designates
//      the CH closest to the BS as a "gateway": every other CH relays its
//      data along the chain toward the gateway, which alone pays the long
//      transmission to the BS. The chain is rebuilt once per setup
//      interval (same cadence as CH selection), so this adds no recurring
//      per-round overhead beyond the relay hops themselves.
//
//   WHY THIS FILE USES A DIFFERENT ENVIRONMENT THAN v3/v5: splitting one
//   transmission of distance D into two hops of D/2 each reduces total
//   amplifier energy only when D exceeds sqrt(2*Eelec/Eamp) -- about 100m
//   in the unified environment's Efs regime, which the unified
//   environment's own BS-at-center geometry (max possible distance 70.7m)
//   never reaches. This file therefore uses the FAR-BS, single-term-radio
//   environment matching LEACH_ORIGINAL.cc (50x50m field, BS at (25,-100),
//   Eamp=100pJ/bit/m^2) -- exactly where a Python pre-check confirmed real
//   distances (mean ~126m) comfortably exceed the ~31.6m break-even point
//   for this radio model, and found a consistent, trade-off-free
//   improvement: FND +25.6%, HND +25.9%, LND +22.4%, PDR +0.9pp versus the
//   same design without chaining. As always, exact resulting numbers
//   should be confirmed by compiling and running this file.
// ============================================================================
//
...
// ----------------------------- Network -------------------------------------
// CHANGED for v6: far-BS scenario (matches LEACH_ORIGINAL's own environment)
// -- this is where CH-to-CH chaining is mathematically justified (see header).
static constexpr uint32_t N = 100;
static constexpr double AREA = 50.0;
static const double ADV_RANGE = AREA * std::sqrt(2.0); // CH advertisement heard by the whole field (unified env)
static constexpr double BSX = 25.0;
static constexpr double BSY = -100.0;
static constexpr double E0 = 0.5;
static constexpr double RANGE = 12.5;         // scaled down for the smaller 50x50 field
...
static constexpr double E_AMP = 100e-12;      // CHANGED for v6: single-term model (matches
                                               // LEACH_ORIGINAL / the far-BS environment)
...
// CHANGED for v6: single-term amplifier model (matches LEACH_ORIGINAL / far-BS env)
static double TxEnergy(uint32_t bits, double d)
{
    if (d <= 0.0) return bits * E_ELEC;
    return bits * (E_ELEC + E_AMP * d * d);
}
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v6: CH-to-CH Chain Relay (Far-BS Scenario)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("hybrid-v6-chain-clustering.xml");
    ...
    ofstream rounds("hybrid-v6-chain-results.csv");
    ofstream energy("hybrid-v6-chain-node-energy.csv");
    ofstream lifetime("hybrid-v6-chain-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: hybrid-v6-chain-results.csv, hybrid-v6-chain-node-energy.csv,\n";
    cout << "             hybrid-v6-chain-node-lifetime.csv, hybrid-v6-chain-clustering.xml\n";
    cout << "========================================\n";
