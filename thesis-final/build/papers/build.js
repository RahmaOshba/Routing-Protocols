...
// ===== cover
c.push(P([t('مراجعة الأبحاث: الأصلية والـ Hybrid والحديثة',{b:true,size:46,c:RED})],{after:80}));
c.push(P([t('9 أبحاث: LEACH · HEED · PEGASIS · SH-LEACH · H-LEACH · EECH-HEED · RL-ILEACH · DL-HEED · TLC-LEACH',{size:26,b:true})],{after:120}));
...
c.push(box('أرقام إعادة البناء بتاعتك',['paper-exact: LND 1327 (البحث 1312، فرق 1.1%)، وFND 1035 (البحث 932). ومن غير حساب طاقة الـ setup، زي ما البحث نفسه عمل.','البيئة الموحدة: FND 1383، وHND 1586، وLND 1842، وPDR 99.40%.'],PINK));
...
c.push(box('أرقام إعادة البناء بتاعتك',['paper-exact (500 نود، 2 J، والـ Sink عند (50,175)، رسالة 125 byte، و5 frames في الـ round): FND 734، وHND 1384، وLND 2259، وPDR 99.80% (البحث: رسومات بس).','البيئة الموحدة (100 نود، 0.5 J، والـ BS في النص): FND 634، وHND 1216، وLND 1883، وPDR 99.68%.','ليه أول نود بتموت بدري؟ المفاوضات بتتكرر كل round: كل نود بتبعت تكلفتها، والمرشحين بيعلنوا نفسهم كل iteration، وكل الجيران بيسمعوا. وده اللي خلاكي في v3 تشيلي المفاوضات وتعيدي استخدام الـ clusters.'],PINK));
c.push(H2('أسئلة متوقعة'));
[['إيه الفرق الأساسي بين LEACH وHEED؟','LEACH بالقرعة ومن غير طاقة وفي خطوة واحدة. HEED باحتمال متناسب مع الطاقة، وبيكرر مع مضاعفة الاحتمال، وبيفض التعادل بتكلفة الاتصال. فالـ CHs أقوى وموزّعة أحسن، بس فيه رسايل أكتر.'],['ليه HEED بيضاعف الاحتمال؟','عشان يخلص في عدد خطوات ثابت (log₂(1/p_min) + 1)، وعشان القوية توصل لـ 1 الأول وتعلن نفسها، والضعيفة تنضم ليها.'],['ليه HEED عندك بيموت بدري مع إن الـ PDR عالي؟','الـ PDR عالي لأن الـ CHs قوية وموزّعة كويس. بس المفاوضات كل round بتصرف طاقة من كل النودز، فأول نود بتموت عند 634.'],['أخدتي إيه من HEED وغيرتي إيه؟','أخدت الطاقة وعدد الجيران. وغيرت إنهم بقوا في معادلة واحدة بتتحسب مرة واحدة من غير مفاوضات، ومعاها إعادة استخدام كل 5 rounds، وضفت الـ Backup اللي HEED نفسه اقترحه.']].forEach(([q,a])=>{c.push(P([t('س: ',{b:true,c:RED}),t(q,{b:true})],{after:30,keepNext:true}));c.push(P([t('ج: ',{b:true,c:RED}),t(a)],{after:120}));});
...
c.push(table(['','LEACH','HEED','SH-LEACH','H-LEACH'],[['أساسه','—','—','LEACH','LEACH'],['أخد من HEED','—','—','C_prob × E/E_max','P × E/E_max'],['الإضافة','T(n) + G','مضاعفة متكررة','× r ÷ (1 + CH_cho mod 1/C)','شرط E > E_avg'],['العيب الأكبر','مش بيبص على الطاقة','overhead','عدد الـ CHs مش مضمون + مفيش مضاعفة','Deadlock + طاقة ثابتة'],['النتيجة المنشورة','FND 932 / LND 1312','رسومات','رسم بس','LND 4312'],['Improved عندك','—','—','r يلف كل 10 rounds: FND 1480','≥ + مجموعة G: FND 1473'],['اتنقل لـ v8','T(r) والـ epoch','الطاقة والجيران','—','I5 (≥ المتوسط)']],[1700,1800,1800,2238,2100],{bf:true,size:17}));
c.push(H2('الأرقام: البحث مقابل إعادة البناء'));
c.push(table(['البروتوكول','البحث (LND)','paper-exact عندك','البيئة الموحدة (FND / HND / LND / PDR)'],[['LEACH','1312','1327','1383 / 1586 / 1842 / 99.40%'],['HEED','رسومات بس','2259','634 / 1216 / 1883 / 99.68%'],['PEGASIS','2192','2186','1324 / 2352 / 3504 / 99.42%'],['SH-LEACH (improved)','رسم بس','689','1480 / 1542 / 1558 / 99.21%'],['H-LEACH (improved)','4312','1207','1473 / 1509 / 1525 / 99.36%'],['EECH-HEED (improved، للسياق)','2200','3783 (بالـ sensing)','1368 / 1385 / 1767 / 99.51%'],['v8-Chain (للمقارنة)','—','—','2621 / 2711 / 2756 / 99.27%']],[2200,1800,2000,3638],{hl:i=>i===6}));
c.push(H2('🔗 جملة الختام'));
c.push(say(['"يعني كل بحث حل جزء: LEACH عمل الـ clustering والدوران بس من غير طاقة، وHEED زود الطاقة بس بمفاوضات مكلّفة، وPEGASIS قلل المسافات بالسلسلة بس بتأخير ونقطة فشل واحدة.','والبحثين الـ Hybrid (SH-LEACH وH-LEACH) حاولوا يدمجوا LEACH وHEED وحسّنوا الاختيار، بس فيهم أخطاء صلّحتها، وحتى بعد التصليح محدش فيهم بيعيد استخدام الـ clusters ولا بيتعامل مع موت الـ CH ولا بيوصّل بين الـ clusters.','والأبحاث الحديثة (EECH-HEED وRL-ILEACH وDL-HEED وTLC-LEACH) ركّزت على اختيار الـ CH بالمناطق أو الذكاء الاصطناعي أو الـ grid، ومحدش فيهم عمل إعادة استخدام وحماية من موت الـ CH وتوصيل بالطاقة مع بعض.','وأنا جمعت الأحسن من الكل: دوران LEACH، وذكاء HEED بحسبة واحدة من غير مفاوضات، وشرط الطاقة من H-LEACH بعد تصليحه، والتوصيل من PEGASIS بس لما يوفّر طاقة. وزودت حاجة محدش فيهم عملها: حماية الـ CH قبل ما يموت وبعده."']));
c.push(H2('⚡ أسئلة سريعة'));
c.push(table(['السؤال','الرد في جملة'],[['EECH-HEED عمره أطول منك؟','في سيناريو البحث بس، لأنه بيبعت أقل (sensing) وطاقته أكتر. في نفس البيئة الـ FND بتاعه 1368 وأنا 2621.'],['ليه مش ذكاء اصطناعي؟','RL-ILEACH أول نود فيه بتموت بدري عن ILEACH، وDL-HEED تدريبه مكلّف وأرقامه متناقضة، وأنا بمعادلة بسيطة وصلت FND 2621.'],['غلطة SH-LEACH؟','رقم الـ round في المعادلة مابيتصفرش، فالاحتمال بيكبر لحد ما يعدّي 1، وبيطلع حوالي 29 CH كل round.'],['غلطة H-LEACH؟','شرط أكبر من المتوسط بيوقّف البروتوكول في أول round (deadlock).'],['الفرق بين LEACH وHEED؟','LEACH بالقرعة ومن غير طاقة، وHEED بالطاقة بس بمفاوضات.'],['ليه PEGASIS مش الأحسن رغم LND أعلى؟','أول نود عنده بتموت قبلي بحوالي 1300 round، وفيه تأخير ونقطة فشل واحدة.'],['ليه HEED عندك بيموت بدري؟','المفاوضات كل round بطاقة 0.5 J بس، فالرسايل بتاكل البطارية.'],['إزاي تأكدتي إن الكود صح؟','بإعدادات كل بحث: LEACH 1327 (البحث 1312)، وPEGASIS 2186 (البحث 2192).'],['ليه d0 = 87.7 مش 75؟','87.7 محسوبة من √(ε_fs/ε_mp) بنفس ثوابت HEED، و75 قيمة ثابتة حطها البحث.']],[3600,6038],{bf:true}));
...
const doc=new Document({creator:'Rahma Khaled Oshba',title:'مراجعة LEACH وHEED وPEGASIS',
 ...
 sections:[{properties:{bidi:true,page:{size:{width:11906,height:16838},margin:{top:1134,bottom:1134,left:1134,right:1134}}},
  footers:{default:new Footer({children:[new Paragraph({bidirectional:true,alignment:AlignmentType.LEFT,children:[new TextRun({children:[PageNumber.CURRENT],font:'Arial',size:16,color:RED,bold:true}),t('   مراجعة الأبحاث',{size:16,c:'888888'})]})]})},
  children:c}]});
