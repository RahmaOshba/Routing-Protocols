...
// ============================================================================
// HYBRID PROTOCOL v6 -- FAR-BS v2 (CH-to-CH chain, NO backup, 100x100 field)
// ns-3.41 / C++
//
...
//
// REVISION NOTE: this replaces the original archive/v6_old_farBS_25_-100.cc,
// which used a smaller 50x50 field with a single-term amplifier model to
// match LEACH_ORIGINAL.cc exactly. That made "Far-BS" not directly
// comparable to the unified-environment (100x100, two-slope radio) runs
// used everywhere else in the thesis. This file keeps the field size
// (100x100), neighbor RANGE (25) and two-slope radio model IDENTICAL to
// v6_chain_center.cc -- the ONLY difference between the two
// files is BS position: here BS = (50,-100), 100m below the field's south
// edge, so every node-to-BS distance (111m-206m) still comfortably exceeds
// the two-slope model's crossover distance d0 = sqrt(Efs/Emp) = 87.7m,
// preserving the physical justification for chaining while making the
// Center/Far comparison a true single-variable (BS location only) test.
//
// PURPOSE: this file is one cell of a controlled 2x2 design isolating two
// independent variables -- Chain (yes/no) and Sink location (center/far):
//
//                    | NO CHAIN                  | CHAIN, NO BACKUP     | CHAIN + BACKUP
//   Sink at CENTER   | v3_single_pass_reuse_int5.cc           | v6_center.cc         | v7_chain_backup_center.cc
//   Sink FAR         | archive/v3_old_farBS_25_-100.cc      | THIS FILE (v6)       | v7_chain_backup_farBS.cc
//
// v6_farBS_v2 vs v7_farBS_v2 isolates exactly the Backup-CH-Failover +
// Chain-Repair contribution, with Chain and environment held fixed.
//
// BUGFIX included in this file (see RunSelection's fallback block below):
// late in a run, every still-alive node can already have served as CH once
// during the current EPOCH window; the old fallback then had nothing left
// to pick from and produced a round with CH_Count = 0 even though nodes
// were alive. Fixed by relaxing the "already served" restriction as a last
// resort so at least one CH always exists while >=1 node is alive.
// ============================================================================
//
...
// ----------------------------- Network -------------------------------------
// 100x100 field, unified two-slope radio, BS moved far below the field --
// this is where the math predicts CH-to-CH chaining SHOULD help (see
// header). Paired with v6_chain_center.cc (same field/radio,
// BS at center) and both v3 baselines for the full 2x2 comparison.
...
static constexpr double BSY = -100.0;
static constexpr double E0 = 0.5;
static constexpr double RANGE = 25.0;         // unified-environment value
...
static constexpr double E_MP = 0.0013e-12;    // REVERTED: unified two-slope model
...
// REVERTED: two-slope radio model (unified environment, matches original V3)
                              ...
                              uint32_t setupIndex, mt19937& rng)
{
    ...
    if (candidates.empty()) {
        ...
        if (eligible.empty()) {
            // BUGFIX (found while diagnosing v7's CH_Count=0 rounds): late in the
            // simulation, EVERY still-alive node can already have served as CH
            // once during the current epoch window, leaving this fallback with
            // nothing to pick from and the round with zero CHs at all -- even
            // though nodes are alive. Relax the "already served this epoch"
            // restriction in that situation only: this guarantees at least one
            // CH exists whenever >=1 node is alive, independent of what happens
            // downstream (backup failover, chain relay, etc.).
...
static RoundResult SimulateRound(vector<SensorNode>& nodes, uint32_t round, bool isSetupRound,
                                 const vector<uint32_t>& chainOrder, uint32_t gatewayCH)
{
    RoundResult r;
    r.round = round;
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v6 FAR-BS v2: CH-to-CH Chain (100x100 field, unified radio, BS far)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("v6_chain_farBS-clustering.xml");
    ...
    ofstream rounds("v6_chain_farBS-results.csv");
    ofstream energy("v6_chain_farBS-node-energy.csv");
    ofstream lifetime("v6_chain_farBS-node-lifetime.csv");
    ...
    for (uint32_t round = 1; round <= MAX_ROUNDS; ++round) {
        ...
        RoundResult r = SimulateRound(nodes, round, isSetupRound, chainOrder, gatewayCH);
    ...
    cout << "\nCSV outputs: v6_chain_farBS-results.csv, v6_chain_farBS-node-energy.csv,\n";
    cout << "             v6_chain_farBS-node-lifetime.csv, v6_chain_farBS-clustering.xml\n";
    cout << "========================================\n";
