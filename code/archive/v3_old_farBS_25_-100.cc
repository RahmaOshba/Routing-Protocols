...
// ============================================================================
// HYBRID PROTOCOL v3 -- FAR-BS BASELINE (fair comparison control for v6)
// ns-3.41 / C++
//
// IDENTICAL algorithm to v3 (single-pass score + multi-round reuse +
// direct CH-to-BS transmission, NO chain) -- but placed in the SAME
// far-BS, single-term-radio environment as archive/v6_old_farBS_25_-100.cc (50x50m field, BS
// far at (25,-100)). This file exists ONLY so that v6's CH-to-CH chain
// mechanism can be evaluated against a true, controlled baseline: same
// environment, same CH-selection logic, same everything except the chain.
// Without this file, comparing v6 against the original v3 (unified,
// BS-at-center environment) would confound two changes at once (the
// environment AND the chain mechanism) -- not a fair test of the chain
// idea itself.
//
// Python pre-check for this exact scenario: FND=971, HND=1261, LND=1406,
// PDR=96.35% (vs. v6_chain's Python pre-check of FND=1220, HND=1588,
// LND=1721, PDR=97.17% -- a +22-26% improvement from the chain alone).
// ============================================================================
//
...
static constexpr double AREA = 50.0;    // CHANGED (was 100): matches V6's far-BS scenario exactly
static const double ADV_RANGE = AREA * std::sqrt(2.0); // CH advertisement heard by the whole field (unified env)
static constexpr double BSX = 25.0;     // CHANGED (was 50): for a controlled, fair comparison
static constexpr double BSY = -100.0;   // CHANGED (was 50): against V6's CH-to-CH chain mechanism
static constexpr double E0 = 0.5;
static constexpr double RANGE = 12.5;   // CHANGED (was 25): scaled down for the smaller 50x50 field
...
static constexpr double E_AMP = 100e-12;  // CHANGED (was E_FS/E_MP two-slope): single term,
                                           // matching V6 exactly for a controlled comparison
...
// (D0/two-slope threshold removed -- this file uses a single-term radio model, matching V6)
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v3 FAR-BS BASELINE (fair comparison control for v6)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("hybrid-v3-farBS-baseline-clustering.xml");
    ...
    ofstream rounds("hybrid-v3-farBS-baseline-results.csv");
    ofstream energy("hybrid-v3-farBS-baseline-node-energy.csv");
    ofstream lifetime("hybrid-v3-farBS-baseline-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: hybrid-v3-farBS-baseline-results.csv, hybrid-v3-farBS-baseline-node-energy.csv,\n";
    cout << "             hybrid-v3-farBS-baseline-node-lifetime.csv, hybrid-v3-farBS-baseline-clustering.xml\n";
    cout << "========================================\n";
