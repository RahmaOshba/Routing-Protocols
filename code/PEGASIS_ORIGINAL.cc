// ============================================================================
// PEGASIS -- ORIGINAL (paper-faithful reproduction)
// ns-3.41 / C++
//
// Matches Lindsey & Raghavendra (2002) as closely as practical:
//   - Chain-based (not cluster-based): each node talks only to its nearest
//     neighbor along a greedily-constructed chain.
//   - Leader (the single node that transmits to the BS each round) rotates
//     round-robin through chain positions ("node i mod N" in the paper).
//   - Token-passing/data-fusion: data flows from BOTH chain ends INWARD
//     toward the leader; each relay hop pays one TX (sender) + one RX +
//     aggregation cost (receiver, fusing incoming data with its own).
//   - Chain is rebuilt (greedy, starting from the node farthest from the
//     BS) whenever a node has died since the last build -- exactly as the
//     paper describes ("when a node dies, the chain is reconstructed").
//   - Single amplifier term E_amp=100pJ/bit/m^2 (same radio model as the
//     LEACH paper, which this paper explicitly reuses) -- NOT the
//     two-slope Efs/Emp/d0 model used in this thesis's UNIFIED environment.
...
static constexpr double AREA = 50.0;   // ORIGINAL: paper's 50x50m field (Table 1)
static constexpr double BSX = 25.0;    // ORIGINAL: paper's stated BS position for this field
static constexpr double BSY = 150.0;   // ("at least 100m from the nearest node")
...
static constexpr double E_AMP = 100e-12;  // J/bit/m^2 (ORIGINAL: single term, same as LEACH paper)
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  PEGASIS -- ORIGINAL (paper-faithful reproduction)\n";
    cout << "========================================\n";
    cout << "Nodes = " << N << " | Area = " << AREA << "x" << AREA << " m | BS = (" << BSX << "," << BSY << ")\n";
    cout << "Initial Energy = " << E0 << " J/node | Radio = single amplifier term (paper's Eq.1)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("pegasis-ORIGINAL-clustering.xml");
    ...
    ofstream rounds("pegasis-ORIGINAL-results.csv");
    ofstream energy("pegasis-ORIGINAL-node-energy.csv");
    ofstream lifetime("pegasis-ORIGINAL-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: pegasis-ORIGINAL-results.csv, pegasis-ORIGINAL-node-energy.csv,\n";
    cout << "             pegasis-ORIGINAL-node-lifetime.csv, pegasis-ORIGINAL-clustering.xml\n";
    cout << "========================================\n";
