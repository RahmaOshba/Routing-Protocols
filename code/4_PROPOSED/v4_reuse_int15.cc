...
// ============================================================================
// HYBRID PROTOCOL v4: MULTI-ROUND CLUSTERING (Energy-Efficient Design, RE-TUNED)
// ns-3.41 / C++
//
...
// RESULT of this exact file (seed 12345): FND 1513 / HND 2158 / LND 2416 / PDR 97.07%
...
//
// SAME algorithm as v3 (single-pass HEED-style selection + multi-round
// reuse) -- see v3's original design rationale below, unchanged. The ONLY
// change in v4 is SETUP_INTERVAL: 5 -> 15 rounds.
//
// WHY THIS RE-TUNING: v3 showed that re-clustering every 5 rounds beats
// re-clustering every round. v4 tests whether stretching the interval
// further (15 rounds) helps even more: control overhead drops, but the
// clusters get older (a CH keeps the role longer). Result: FND drops and
// PDR drops (more members orphaned when a CH dies mid-interval), so the
// thesis keeps interval = 5 from v5-exp onwards.
// (An earlier version of this header cited EECH-HEED 3808 and PEGASIS
// 2887 as the motivation; those numbers came from old, since-corrected
// codes and are no longer valid.)
//
...
static constexpr uint32_t SETUP_INTERVAL = 15; // RE-TUNED (was 5): a Python sweep found stretching the
                                                // setup interval further reduces control overhead even
                                                // more, extending LND substantially -- confirmed robust
                                                // across two different random seeds (not a one-off spike).
                                                // This directly responds to EECH-HEED-Improved and
                                                // PEGASIS-Edited overtaking the original V3 (interval=5)
                                                // after their own improvements.
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v4: Multi-Round Clustering (Re-tuned interval=15)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("v4_reuse_int15-clustering.xml");
    ...
    ofstream rounds("v4_reuse_int15-results.csv");
    ofstream energy("v4_reuse_int15-node-energy.csv");
    ofstream lifetime("v4_reuse_int15-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: v4_reuse_int15-results.csv, v4_reuse_int15-node-energy.csv,\n";
    cout << "             v4_reuse_int15-node-lifetime.csv, v4_reuse_int15-clustering.xml\n";
    cout << "========================================\n";
