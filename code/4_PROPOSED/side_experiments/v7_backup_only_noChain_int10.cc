...
// ============================================================================
// HYBRID PROTOCOL v7 -- CENTER, NO CHAIN, 10-ROUND INTERVAL: BACKUP CH
// FAILOVER ONLY (direct CH->BS)
// ns-3.41 / C++
//
...
// RESULT of this exact file (seed 12345): FND 1540 / HND 2090 / LND 2291 / PDR 98.58%
...
//
// VARIANT requested to isolate two things at once against
// v7_chain_backup_center.cc: (1) what backup-only (no chain) looks like -- direct CH->BS
// transmission, exactly like v5's design, but with the two bugfixes below
// carried over -- and (2) a longer 10-round setup interval instead of 5
// (half the control-message overhead per unit time, at the cost of staler
// clustering, the same v3->v4 trade-off already documented in the thesis).
// Same 100x100 field, RANGE=25, two-slope radio, BS at field center (50,50)
// as v7_chain_backup_center.cc -- ONLY the chain mechanism (removed) and SETUP_INTERVAL
// (5->10) differ, so comparing against v7_center isolates the chain's own
// contribution, and comparing against v5_backup_int15.cc (interval=15) isolates
// the interval choice.
//
...
// simply takes over the dead CH's own direct-to-BS role -- there is no
// chain to repair in this variant, so promotion is now a matter of
// keeping *a* CH alive for that cluster, nothing more.
//
...
static constexpr uint32_t SETUP_INTERVAL = 10; // CHANGED for this variant: re-cluster every 10 rounds (was 5)
...
// REMOVED for this variant: no CH-to-CH chain. CHs transmit their
// aggregated cluster data directly to the BS, exactly as in v3/v5 -- see
// v7_chain_backup_center.cc for the chain mechanism this variant
// deliberately omits.
                                 ...
                                 uint32_t setupIntervalLen)
{
    ...
    for (uint32_t c : orphanedCHs) {
        ...
        if (backupId != numeric_limits<uint32_t>::max()
            ...
            if (nodes[backupId].energy >= estCostPerRound) {
                ...
                promoted = true;
                // No chain to repair in this variant -- the backup simply
                // becomes the cluster's new direct-to-BS transmitter.
            } else {
        ...
        if (!promoted) {
            ...
            // No backup or emergency CH available -- this cluster's members
            // are simply unclustered for the rest of the interval.
        }
    }
    for (auto& n : nodes) {
        if (n.alive && n.clusterHead < N && !nodes[n.clusterHead].alive)
    ...
    vector<uint32_t> chMsgCount(N, 0);
    ...
    for (uint32_t c = 0; c < N; ++c) {
        ...
        nodes[c].energy -= agg;
        chMsgCount[c] = members + 1;
    }

    // ---- NO CHAIN in this variant: every CH transmits its aggregated
    // cluster data directly to the BS, exactly as in v3/v5. ----
    for (uint32_t c = 0; c < N; ++c) {
        if (!nodes[c].alive || !nodes[c].finalCH) continue;
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v7 CENTER, NO CHAIN, interval=10: Backup CH Failover only (direct CH->BS)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("v7_backup_only_noChain_int10-clustering.xml");
    ...
    ofstream rounds("v7_backup_only_noChain_int10-results.csv");
    ofstream energy("v7_backup_only_noChain_int10-node-energy.csv");
    ofstream lifetime("v7_backup_only_noChain_int10-node-lifetime.csv");
    ...
    rounds << "Round,Alive,Dead,CH_Count,Generated,Delivered,Lost,Unclustered,"
              ...
              "PDR,Round_Duration_s,Throughput_kbps,Backup_Promotions,Rejected_Promotions,Partial_Promotions,Emergency_Promotions\n";
    ...
    uint64_t totalPromotions = 0, totalRejected = 0;
    ...
    for (uint32_t round = 1; round <= MAX_ROUNDS; ++round) {
        ...
        RoundResult r = SimulateRound(nodes, round, isSetupRound, SETUP_INTERVAL);
    ...
    cout << "\nCSV outputs: v7_backup_only_noChain_int10-results.csv, v7_backup_only_noChain_int10-node-energy.csv,\n";
    cout << "             v7_backup_only_noChain_int10-node-lifetime.csv, v7_backup_only_noChain_int10-clustering.xml\n";
    cout << "========================================\n";
