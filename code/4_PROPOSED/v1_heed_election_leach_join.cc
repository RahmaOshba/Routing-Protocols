...
// ============================================================================
// HYBRID PROTOCOL v1: LEACH clustering + HEED CH selection (CLEAN VERSION)
// ns-3.41 / C++
//
...
// RESULT of this exact file (seed 12345): FND 322 / HND 530 / LND 1217 / PDR 99.75%
...
//
// DESIGN:
//   PHASE 1 -- CH Selection: taken from "Original HEED" (heed_COMBINED_FIX.cc)
//       unchanged -- residual-energy-based probability, iterative doubling,
//       local competition using cost = 1/(degree+1) (favors well-connected
//       hub nodes; see that file's header for the full bug-fix rationale).
//   PHASE 2 -- Cluster formation (the actual DIFFERENCE from HEED):
//       Original HEED picks the LOWEST-COST reachable CH for each member
//       (BestCH(), degree-based tie-break). This Hybrid instead uses
//       LEACH's simple rule: each member joins whichever alive CH is
//       NEAREST by Euclidean distance -- no cost/degree involved in the
//       join decision at all, only in who becomes CH in Phase 1.
//
// This is the RELIABLE version: analytical energy model + simple NetAnim
// node-color visualization only (no WiFi/packet layer, which proved
// fragile/slow due to the shared-collision-domain issue). Exports
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v1: LEACH clustering + HEED CH selection\n";
    cout << "========================================\n";
    cout << "Cprob=" << CPROB << " pMin=" << PMIN << " | Selection cost=1/(degree+1)\n";
    ...
    AnimationInterface anim("v1_heed_election_leach_join-clustering.xml");
    ...
    ofstream rounds("v1_heed_election_leach_join-results.csv");
    ofstream energy("v1_heed_election_leach_join-node-energy.csv");
    ofstream lifetime("v1_heed_election_leach_join-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: v1_heed_election_leach_join-results.csv, v1_heed_election_leach_join-node-energy.csv,\n";
    cout << "             v1_heed_election_leach_join-node-lifetime.csv, v1_heed_election_leach_join-clustering.xml\n";
    cout << "========================================\n";
