// ============================================================================
// SH-LEACH (2015) -- EDITED + IMPROVED (unified environment + one fix)
// ns-3.41 / C++
//
// hybrid_SHLEACH_EDITED_unified.cc with ONE change aimed at the weakness the
// paper itself admits ("does not ensure the number of cluster heads"):
// in Eq. 3 the factor (Cprob * r) grows without bound, so the probability is
// ~0 in the first rounds and > 1 later (almost every node becomes CH).
// IMPROVED: the round factor restarts every 1/Cprob rounds, exactly like
// LEACH's epoch:
//     (Cprob * r)  ->  Cprob * ( ((r - 1) mod (1/Cprob)) + 1 )
// so the probability ramps up inside each epoch and resets, keeping the CH
// count bounded. Cprob stays at the paper's 0.10, so the gain comes from
// this single fix (a sweep of Cprob = 0.02-0.20 gives FND 1394-1512, all far
// above the un-improved 647).
// ============================================================================
...
#ifndef SH_CPROB
#define SH_CPROB 0.10
#endif
static constexpr double CPROB = SH_CPROB; // paper's Cprob (kept)
...
static constexpr double E_MP = 0.0013e-12; // J/bit/m^4 (UNIFIED)
...
static uint32_t RunSHLEACH(vector<SensorNode>& nodes, uint32_t round, mt19937& rng)
{
    ...
    for (auto& n : nodes) {
        if (!n.alive) continue;
        const double rEpoch = static_cast<double>(((round - 1) % period) + 1);   // IMPROVED: epoch-periodic round factor
        const double prob = min(1.0, CPROB * (n.energy / EMAX) * (CPROB * rEpoch) / denom);
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID PROTOCOL (Literature): SH-LEACH EDITED+IMPROVED (epoch-periodic round factor)\n";
    cout << "========================================\n";
    cout << "Nodes = " << N << " | Area = " << AREA << "x" << AREA << " m\n";
    cout << "Cprob = " << CPROB << " | Emax = " << EMAX << " J | Radio: two-slope (unified)\n";
    ...
    AnimationInterface anim("shleach_IMPROVED-clustering.xml");
    ...
    ofstream rounds("shleach_IMPROVED-results.csv");
    ofstream energy("shleach_IMPROVED-node-energy.csv");
    ofstream lifetime("shleach_IMPROVED-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: shleach_IMPROVED-results.csv, shleach_IMPROVED-node-energy.csv,\n";
    cout << "             shleach_IMPROVED-node-lifetime.csv, shleach_IMPROVED-clustering.xml\n";
    cout << "========================================\n";
