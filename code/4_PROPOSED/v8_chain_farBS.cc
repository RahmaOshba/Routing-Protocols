...
// ============================================================================
// HYBRID PROTOCOL v8-Chain: v8 + energy-aware CH-to-CH relay. v8 = v5b (Backup Failover + Chain/Cluster Repair +
...
// RESULT of this exact file (seed 12345): FND 1671 / HND 1851 / LND 1906 / PDR 99.36%
...
struct RoundResult {
    ...
    uint32_t relayed = 0;            // v8-Chain: CH packets forwarded to another CH
};
...
#define BS_Y -100.0
#endif
static constexpr double BSY = BS_Y;          // 50 = field center, -100 = far BS (thesis Far-BS placement)
...
#ifndef TOPO_SEED
#define TOPO_SEED 12345
#endif
static constexpr uint32_t SEED = TOPO_SEED;   // compile with -DTOPO_SEED=... for the robustness runs
...
// ---- v8-Chain: how a CH's aggregated packet reaches the BS ----
//   0 = direct CH -> BS (plain v8)
//   1 = ordered CH-to-CH chain (thesis v6/v7 style): CHs sorted by distance to the BS,
//       each forwards to the next one, the CH closest to the BS is the gateway
//   2 = energy-aware relay: a CH forwards to another CH that is closer to the BS ONLY if
//       that costs less total energy than sending to the BS itself; otherwise direct
#ifndef CHAIN_MODE
#define CHAIN_MODE 2
#endif
static constexpr int ROUTING = CHAIN_MODE;
...
// are alive NOW, so a promoted backup / handover CH automatically takes its predecessor's
// place in the relay path (chain repair with no extra messages: CH positions are known
// from the setup advertisements).
static vector<int32_t> ComputeNextHop(const vector<SensorNode>& nodes)
{
    vector<int32_t> next(N, -1);
    if (ROUTING == 0) return next;
    vector<uint32_t> chs;
    for (const auto& n : nodes) if (n.alive && n.finalCH) chs.push_back(n.id);
    if (ROUTING == 1) {
        sort(chs.begin(), chs.end(), [&](uint32_t a, uint32_t b) { return DistBS(nodes[a]) > DistBS(nodes[b]); });
        for (size_t k = 0; k + 1 < chs.size(); ++k) next[chs[k]] = static_cast<int32_t>(chs[k + 1]);
        return next;
    }
    ...
    for (uint32_t c : chs) {
        ...
        for (uint32_t j : chs) {
            ...
            if (viaJ < best) { best = viaJ; next[c] = static_cast<int32_t>(j); }
        }
    }
    return next;
}
...
static bool ProactiveHandover(vector<SensorNode>& nodes, uint32_t ch, RoundResult& r)
{
    ...
    double farthest = 0.0;   // the announcement must reach every member of the cluster
    for (const auto& m : nodes)
        if (m.alive && m.clusterHead == ch && m.id != nb) farthest = max(farthest, Dist(m, nodes[nb]));
    const double announce = TxEnergy(CONTROL_BITS, farthest);
...
static RoundResult SimulateRound(vector<SensorNode>& nodes, uint32_t round, bool isSetupRound)
{
    ...
    if (PROACTIVE_HANDOVER) {
        ...
        const vector<int32_t> hop = ComputeNextHop(nodes);
        vector<uint32_t> incoming(N, 0);
        for (uint32_t c = 0; c < N; ++c) if (hop[c] >= 0) ++incoming[hop[c]];
        for (uint32_t c = 0; c < N; ++c) {
            if (!nodes[c].alive || !nodes[c].finalCH) continue;
    ...
    // ---- CH phase: aggregate and forward, farthest-from-BS first so relayed data arrives in time ----
    const vector<int32_t> nextHop = ComputeNextHop(nodes);
    vector<uint32_t> order;
    for (uint32_t c = 0; c < N; ++c) if (nodes[c].alive && nodes[c].finalCH) order.push_back(c);
    if (ROUTING != 0)   // relays must be processed before the CH they forward to
        sort(order.begin(), order.end(), [&](uint32_t a, uint32_t b) { return DistBS(nodes[a]) > DistBS(nodes[b]); });
    vector<uint32_t> inPackets(N, 0), inSources(N, 0);
    vector<double> inDelay(N, 0.0);            // summed delay-so-far of the relayed sources

    vector<bool> done(N, false);
    auto processCH = [&](uint32_t c) {
        if (!nodes[c].alive || !nodes[c].finalCH || done[c]) return;
        done[c] = true;
        const uint32_t members = membersPerCH[c];
        const double agg = static_cast<double>(members + 1 + inPackets[c]) * AggEnergy(PACKET_BITS);   // members + own + relayed
        if (agg > nodes[c].energy) { nodes[c].energy = 0.0; nodes[c].alive = false; nodes[c].finalCH = false; TryFailover(nodes, c, r); ++r.lost; return; }
        nodes[c].energy -= agg;
        ...
        nodes[c].energy -= tx;
        ++r.dataTx;
        const double hopDelay = DelayMs(PACKET_BITS, d);
        const uint32_t sources = members + 1 + inSources[c];
        double srcDelay = inDelay[c] + (1 + inSources[c]) * hopDelay;
        for (uint32_t i = 0; i < N; ++i)
            if (memberDelivered[i] && nodes[i].clusterHead == c) srcDelay += delayToCH[i] + hopDelay;
        if (dst < 0) {                            // reached the BS
            ++r.dataRx;
            deliveredSources += sources;
            delaySum += srcDelay;
            return;
        }
        const uint32_t nx = static_cast<uint32_t>(dst);
        const double rx = RxEnergy(PACKET_BITS);
        if (!nodes[nx].alive || !nodes[nx].finalCH || rx > nodes[nx].energy) return;    // relay lost
        nodes[nx].energy -= rx;
        ++r.dataRx; ++r.relayed;
        ++inPackets[nx];
        inSources[nx] += sources;
        inDelay[nx] += srcDelay;
    };

    if (ROUTING != 0) for (uint32_t c : order) processCH(c);
    // backups promoted during this phase (not in `order`) still send this round, like v8
    for (uint32_t c = 0; c < N; ++c) processCH(c);
...
int main(int argc, char* argv[])
{
    ...
    AnimationInterface anim("v8_chain_farBS-clustering.xml");
    ...
    ofstream rounds("v8_chain_farBS-results.csv");
    ofstream energy("v8_chain_farBS-node-energy.csv");
    ofstream lifetime("v8_chain_farBS-node-lifetime.csv");
    ...
    rounds << "Round,Alive,Dead,CH_Count,Generated,Delivered,Lost,Unclustered,"
              ...
              "Proactive_Handovers,Direct_To_BS,CH_Relays\n";
    ...
    for (uint32_t round = 1; round <= MAX_ROUNDS; ++round) {
        ...
        totalRelayed += r.relayed;               // v8-Chain
        ...
        rounds << fixed << setprecision(10)
               ...
               << r.proactiveHandovers << ',' << r.directToBS << ',' << r.relayed << '\n';
    ...
    cout << "CH-to-CH Relays = " << totalRelayed << "\n";
    cout << "BS = (" << BSX << ", " << BSY << "), routing mode = " << ROUTING << "\n";
    cout << "\nCSV outputs: v8_chain_farBS-results.csv, v8_chain_farBS-node-energy.csv,\n";
    cout << "             v8_chain_farBS-node-lifetime.csv, v8_chain_farBS-clustering.xml\n";
    cout << "========================================\n";
