...
// ============================================================================
// HYBRID PROTOCOL v6 -- SINK-CENTER VARIANT (CH-to-CH chain, unified env.)
// ns-3.41 / C++
//
// IDENTICAL chain mechanism to archive/v6_old_farBS_25_-100.cc (see that file's header for the
// full design rationale) -- but placed in the UNIFIED environment (Sink at
// field center, two-slope radio) instead of the far-BS environment.
//
// PURPOSE: this file completes a full 2x2 comparison isolating the two
// variables (chain vs. no chain) x (Sink center vs. Sink far):
//
//                    | NO CHAIN                  | WITH CHAIN
//   Sink at CENTER   | v3_single_pass_reuse_int5.cc (2703)    | THIS FILE
//   Sink FAR         | archive/v3_old_farBS_25_-100.cc      | archive/v6_old_farBS_25_-100.cc
//
// EXPECTED RESULT: a Python pre-check found that splitting one
// transmission into two hops only saves amplifier energy once the direct
// distance exceeds roughly 100m (the unified environment's Efs regime) --
// but the unified environment's own BS-at-center geometry never produces
// a distance beyond ~70.7m. This file is therefore expected to show LITTLE
// OR NO improvement over v3_single_pass_reuse_int5.cc (possibly a slight regression,
// since every relay hop adds a fixed Eelec electronics cost that isn't
// compensated by amplifier savings at these short distances) -- confirming
// that the chain's benefit (seen in archive/v6_old_farBS_25_-100.cc) is conditional on BS
// placement, not a universal improvement.
// ============================================================================
//
...
// help (see header). Paired with archive/v6_old_farBS_25_-100.cc (far-BS) and both farBS/
// sinkCenter baselines for the full 2x2 comparison.
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n========================================\n";
    cout << "  HYBRID v6 SINK-CENTER VARIANT: CH-to-CH Chain (unified env., control test)\n";
    cout << "========================================\n";
    ...
    AnimationInterface anim("hybrid-v6-chain-sinkCenter-clustering.xml");
    ...
    ofstream rounds("hybrid-v6-chain-sinkCenter-results.csv");
    ofstream energy("hybrid-v6-chain-sinkCenter-node-energy.csv");
    ofstream lifetime("hybrid-v6-chain-sinkCenter-node-lifetime.csv");
    ...
    cout << "\nCSV outputs: hybrid-v6-chain-sinkCenter-results.csv, hybrid-v6-chain-sinkCenter-node-energy.csv,\n";
    cout << "             hybrid-v6-chain-sinkCenter-node-lifetime.csv, hybrid-v6-chain-sinkCenter-clustering.xml\n";
    cout << "========================================\n";
